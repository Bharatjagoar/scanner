...
# ── Expiry normaliser ──────────────────────────────────────────────────────────
def _expiry_str(val):
    """Convert epoch-ms int OR 'YYYY-MM-DD' string to 'YYYY-MM-DD' string."""
...
def _fetch_daily_data(instrument_key, n_days=5):
    ...
    try:
        ...
        # Extract close (index 4) and volume (index 5)
...
def _scan_trending_stocks():
    ...
    Adds a 5-day Volume SMA filter.
    """
    ...
    for s in stocks:
        # Fetch 5 days of data for the SMA
...
def _tr_enter(symbol, fut_key, lot_size, side, ltp, today):
    ...
    print(
        f"  [TR] FUTURES ENTRY {symbol} {side} @ {ltp} | lot={lot_size} "
    ...
    for s in _tr_all_active():
        ...
        try:
            ...
            if result:
                ...
                print(
                    f"  [TR] EXIT {sym} @ {exit_p} {result} "
                    f"| P&L={pnl_pts:+.2f}pts Rs{pnl_inr:+.0f} (lot={lot_size})"
                )
        except Exception as e:
    ...
    while True:
        ...
        for s, side in all_stocks:
            ...
            try:
                ...
                if ltp > capital:
                    print(f"  [EQ] {sym} LTP Rs{ltp} > capital Rs{capital}, skipping")
                    continue
                ...
                if not entered and EQ_ENTRY_START <= cm <= EQ_ENTRY_END:
                    ...
                    if not existing:
                        ...
                        if trigger:
                            ...
                            print(
                                f"  [EQ] ENTRY {sym} {side} @ Rs{ltp} qty={qty} "
                                f"capital=Rs{capital_used} SL={sl_price} TGT={target_price}"
                            )
                ...
                if trade and trade["result"] == "OPEN":
                    ...
                    if result:
                        ...
                        print(
                            f"  [EQ] EXIT {sym} @ Rs{exit_p} {result} "
                            f"P&L=Rs{pnl_inr:+.2f} ({pnl_pct:+.2f}%)"
                        )
...
if __name__ == "__main__":
    ...
    print(f"Market open: {is_market_open()} | FnO Balance: Rs{INITIAL_BALANCE:,.0f}")
